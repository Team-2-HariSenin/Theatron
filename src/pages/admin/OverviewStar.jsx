...
const OverviewStar = () => {
  ...
  const getStars = async (keyword = "", page = 1) => {
    try {
      const response = await axios.get(
        `https://theatron-backend.vercel.app/api/admin/all-star?keyword=${keyword}&page=${page}&limit=10`,
        {
          headers: {
      ...
      setStarData(response.data.data.stars);
  ...
  const handleSearch = () => {
    setCurrentPage(1);
    getStars(searchInput, 1);
  };

  const deleteStar = async (id) => {
    try {
      const response = await axios.delete(
        `https://theatron-backend.vercel.app/api/admin/delete-star/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDelete = (id) => {
    confirm("Do you really want to delete this star?") && deleteStar(id);
  };
            ...
            <tbody>
              {starData.length > 0 ? (
                starData.map((star, index) => (
                  <tr
                    key={star.id}
                    className={`bg-white ${index !== starData.length - 1 ? "border-b" : ""}`}
                  >
                    <td className="px-6 py-4 text-center">{star.id}</td>
                    <td className="text-ellipsis whitespace-nowrap text-nowrap px-6 py-4">
                      {star.name}
                    </td>
                    <td className="px-6 py-4 text-center">
                      {Number(star.total_movie)}
                    </td>
                    <td className="flex items-center justify-center gap-2 px-6 py-4">
                      <Link
                        to={`/admin/stars/edit?id_star=${star.id}`}
