...
const OverviewDirector = () => {
  ...
  const getDirectors = async (keyword = "", page = 1) => {
    try {
      ...
      setDirectorData(response.data.data.directors);
  ...
  const handleDelete = (id) => {
    confirm("Do you really want to delete this director?") &&
      deleteDirector(id);
  };
            ...
            <tbody>
              {directorData.length > 0 ? (
                directorData.map((director, index) => (
                  <tr
                    key={director.id}
                    className={`bg-white ${index !== directorData.length - 1 ? "border-b" : ""}`}
                  >
                    <td className="px-6 py-4 text-center">{director.id}</td>
                    <td className="text-ellipsis whitespace-nowrap text-nowrap px-6 py-4">
                      {director.name}
                    </td>
                    <td className="px-6 py-4 text-center">
                      {Number(director.total_movie)}
                    </td>
                    <td className="flex items-center justify-center gap-2 px-6 py-4">
                      <Link
                        to={`/admin/directors/edit?id_director=${director.id}`}
...
export default OverviewDirector;
