...
const OverviewUser = () => {
  ...
  const getUsers = async (keyword = "", page = 1) => {
    try {
      const response = await axios.get(
        `https://theatron-backend.vercel.app/api/admin/all-user?keyword=${keyword}&page=${page}&limit=10`,
        {
          headers: {
  ...
  const handleSearch = () => {
    setCurrentPage(1);
    getUsers(searchInput, 1);
  };

  return (
    <div className="flex h-screen w-full flex-col gap-4 px-6 pb-10 pt-2">
      <nav className="justify-usert relative ml-5 flex h-fit items-center gap-3 text-4xl font-bold">
        <h1>User</h1>
        <span>
          <svg
            ...
            <tbody>
              {userData.length > 0 ? (
                userData.map((user, index) => (
                  <tr
                    key={user.id}
                    className={`bg-white ${index !== userData.length - 1 ? "border-b" : ""}`}
                  >
                    ...
                    <td className="px-6 py-4 text-center">{user.total_rate}</td>
