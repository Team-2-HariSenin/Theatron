...
const Main = () => {
  ...
  const getMovies = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:3000/api/movie/playnow`,
      );
  ...
  const reorderMovies = (index) => {
    const part1 = movies.slice(index + 1, movies.length); // Index 3 to 6
    const part2 = movies.slice(0, index + 1); // Index 0 to 2
    return [...part1, ...part2];
  };
  ...
  useEffect(() => {
    if (movies.length > 0) {
      ...
      console.log(reorderedMovies);
    }
  }, [movies, currentIndex]);

  return (
    <main>
      <div className="lg: mx-auto mt-9 grid w-full grid-cols-[100%] justify-stretch px-2 lg:container">
                ...
                onSlideChange={(swiper) => {
                  setCurrentIndex(
                    swiper.activeIndex == 0 ||
                      swiper.activeIndex == movies.length - 1
                      ? 0
                      : swiper.activeIndex === 1 ||
                          swiper.activeIndex === movies.length
                        ? 1
                        : swiper.activeIndex,
                  );
                  console.log(swiper);
                  ...
                  movies.map((movie, index) => (
                    ...
                    >
                      ...
                      <div className="relative w-full pb-[70px]">
                        ...
                        <div className="bg-banner-section group absolute bottom-0 h-full w-full cursor-pointer">
                    ...
                    {reorderedMovies.length > 0 &&
                      reorderedMovies.map((movie) => (
                        ...
                        >
                          ...
                          <a className="group absolute left-[94px] right-0 top-0 cursor-pointer overflow-hidden px-2 pt-4 text-white xl:left-[120px]">
